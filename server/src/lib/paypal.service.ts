 ...
 *
 */
const verifyNewTransaction = async (orderModel: Repository<OrderEntity>, paypalTransactionId: string) => {
  try {
    const orders = await orderModel.find({
      where: { paymentResult: { paymentId: paypalTransactionId } },
    });
...
const verifyPayPalPayment = async (paypalTransactionId: string) => {
  ...
  return {
    ...
    value: paypalData.purchase_units[0]?.amount?.value as string,
  };
};

export { verifyPayPalPayment, verifyNewTransaction };
